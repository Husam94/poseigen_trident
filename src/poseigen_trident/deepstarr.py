...
#     def __init__(self, dim_i = (1, 249, 4), 
#                         dim_o = (1, 1, 1), 
#                         ck_i = 7, ck_other = [3, 5, 3],
#                         cf_i = 256, cf_other = [60, 60, 120], 
#                         num_convs = 4, # THE MAXIMUM IS 4.
#                         num_dense = 2, 
...
#         # CANNOT DO DOUBLE STRAND AT SAME TIME. 


#         cks = [ck_i] + ck_other
#         cfs = [dim_i[0], cf_i] + cf_other

#         length_afterconv = dim_i[1]
#         conv_layers = []
...
#             length_afterconv = length_afterconv + (convpad * 2) - cks[i] + 1
...
#             length_afterconv = int(np.ceil((length_afterconv - 2) / 2)) + 1
...
#         for i in np.arange(tot_dense): 
#             dko = length_afterconv if i == 0 else 1
#             dfi = cfs[num_convs] if i == 0 else 256
#             dfo = dim_o[0] if i == tot_dense - 1 else 256

#             dense_layers.append(nn.Conv2d(dfi, dfo, (dko, 1), padding='valid', 
#                                             bias = True, stride = (1,1)))
            
#             if i == tot_dense - 1: break
#             else: 

...
def BatchMaker(trange, trem, tspl, batchsize,
    ...
    if WBG == None:
        ...
        if use_harp:
            scramble = harp.Harpoon(tweights, select = batchsize,
...
def EpochUndersampler(obs_weight, EUS, Split, indivflips, batchsize, tbatch_prop):
    ...
    if obs_weight is not None:
        ...
        if isinstance(obs_weight[0], np.integer): 
            subs_mode = [se.SubSample_Select, {'select_mode': [harp.Harpoon, {}]}]
    ...
    new_split = su.SubSplitGen(obs_weight, Split, onlyfirst = True, proportion = EUS,
...
def _all_same_outputs(pred_mbs):
    if torch.is_tensor(pred_mbs):
        flat = pred_mbs.reshape(-1)
        return bool(flat.numel() > 0 and torch.all(flat == flat[0]).item())
    arr = np.asarray(pred_mbs)
    flat = arr.reshape(-1)
    return bool(flat.size > 0 and np.all(flat == flat[0]))


def EndEpoch(
    *,
    model, metrics, e, best, counter, smallest,
    pnMo, pnMe, pn, patience,
    pred_mbs, duds, mod_init_mode,
    statusprints, savebytrain, poors
):
    stop = False

    if e <= 1:
        best = np.nan_to_num(metrics["Validation"][e])

    val = metrics["Validation"][e]
    improved = best > val if smallest else best < val
    ...
    elif _all_same_outputs(pred_mbs):
        print("SAME OUTPUTS")
        if duds > 0 and mod_init_mode is not None:
            maybe_model = mod_init_mode[0](model, **mod_init_mode[1])
    ...
    elif e > 0 and metrics["Validation"][e] == metrics["Validation"][e - 1]:
        print("STUCK")
        print("POOR MODEL, TERMINATED")
        counter = patience
        stop = True
    ...
    if statusprints is not None and e % statusprints == 0:
        print(f"E {e+1} Training: {metrics['Train'][e]} Validation: {val} Counter {counter}")

    if savebytrain:
        pnMo_train = pn + "_Mod_TRAIN.pt"
        best_train = np.nanmin(metrics["Train"]) if smallest else np.nanmax(metrics["Train"])
        if metrics["Train"][e] == best_train:
            torch.save(model, pnMo_train)

    if poors is not None and e >= poors[0] - 1:
        cuti = val >= poors[1] if smallest else val <= poors[1]
        if cuti:
            print("POOR MODEL, TERMINATED")
            counter = patience
            stop = True
        
    if stop: metrics["terminated"] = True

    # Save duds for pickup
    metrics["duds"] = int(duds)
    
    se.PickleDump(metrics, pnMe)

    return model, best, counter, duds, stop
...
def TridentTrainer(
    ...
    if duds is None: duds = 0
    
    metrics = {'Train': [], 'Validation': [], 'duds': int(duds)}
    counter = 0
    e = 0
    ...
    if pickup and os.path.isfile(pnMe):
        try:
            ...
            duds = int(metrics.get('duds', duds))
            metrics['duds'] = duds
        ...
        if metrics.get('terminated', False):
            print('Was previously terminated')
            return (metrics, model) if returnmodel else metrics
    ##########################################
    ...
    while counter < patience - 1 and e < maxepochs - 1: 
        ...
        model, best, counter, duds, stop = EndEpoch(
            model=model, metrics=metrics, e=e, best=best, counter=counter, smallest=smallest,
            pnMo=pnMo, pnMe=pnMe, pn=pn, patience=patience,
            pred_mbs=pred_mbs, duds=duds, mod_init_mode=mod_init_mode,
            statusprints=statusprints, savebytrain=savebytrain, poors=poors)
        if stop: break
        e += 1
...
def EpochUndersampler_BW(bw_train, tspl, bind_train, bin_sizes,
    ...
    if EUS_mode == 'harp': 
        harp_samp = harp.Harpoon_torch(tra_out, select = EUS_prop, repeat=1, num_bins = 500)
        new_tspl = tspl[harp_samp.cpu()]
    ...
    if EUS_mode == 'opt': 
        ...
        opt_samp = bu.OptSample(bind_train, bin_sizes=bin_sizes, select=EUS_prop,
...
def TridentTrainer_BW(
    ...
    inps, out, out_std = None, 
    out_bind = None, bin_sizes = None, bw_mode = 'recip', 

    Split = None, flips = None, 
    ...
    batchsize = 128, batchsize_infer = None, opt = Adam, learningrate = 0.001, maxepochs = 20, patience = 5, 
    pathname = None, statusprints = True, returnmodel = False, pickup = False,
    savebytrain = False, seed = None, dtypo = torch.float
    ):
    ...
    bw_base = {x: bu.BinWeights(out_bind[y], bin_sizes = bin_sizes, mode = bw_mode)
    ...
    while counter < patience - 1 and e < maxepochs - 1: 
        ...
        if EUS_prop is not None: 
            ...
            tspl = EpochUndersampler_BW(bw_train, tspl, out_bind[tspl], bin_sizes,
                                        ...
                                        replace = EUS_replace, tra_out = tra_out)
        
        #-------------------------------------------
        tra_bind = out_bind[tspl] 
        tra_wei = bu.BinWeights(tra_bind, bin_sizes = bin_sizes, mode = bw_mode)
...
def BinnedLoss(inp1, inp2, std = None, 
    ...
    for ip, bi, un, bxo in zip(inp_pairs, bind, uni, bin_sizes):
        ...
        par = bu.BinParser(bi.reshape(-1), uni = un, categorical = True)
...
class PerDimAttentionPooling(nn.Module):
    ...
    def reset_parameters(self):
        # Same initialization style PyTorch uses for Linear weights
        nn.init.kaiming_uniform_(self.w.unsqueeze(0), a=5**0.5)
    ...
    def forward(self, x):
        ...
        # Attention logits: (N, E, L)
        logits = x * self.w.unsqueeze(-1)

        # Softmax over sequence positions
        weights = F.softmax(logits, dim=-1)

        # Weighted sum over L
        pooled = torch.sum(weights * x, dim=-1)   # (N, E)

        return pooled.unsqueeze(-1).unsqueeze(-1)


class DiagonalAttentionPooling(nn.Module):
    """
    Dimension-wise attention pooling (diagonal mode from Balci et al.)
    Input:  (N, E, L, 1)
    Output: (N, E, 1, 1), (N, E, L)
    """
    def __init__(self, embed_dim, init_constant=2):
        super().__init__()
        self.feature_size = embed_dim

        # One learnable weight per channel (diagonal mode)
        self.weight = nn.Parameter(torch.empty(embed_dim))

        # Initialize to constant * ones (matches Balci implementation)
        with torch.no_grad():
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_uniform_(self.weight.unsqueeze(0), a=5**0.5)
    ...
    def forward(self, x):
        ...
        # mask[b, c, l] = x[b, c, l] * weight[c]
        mask = torch.einsum("bcl, c -> bcl", x, self.weight)

        # Softmax over sequence length
        mask = self.softmax(mask)   # (N, E, L)

        # Weighted sum across L
        pooled = torch.sum(x * mask, dim=-1)   # (N, E)

        # Restore shape
        pooled = pooled.unsqueeze(-1).unsqueeze(-1)

        return pooled


class RegularAttentionPooling(nn.Module):
    """
    Standard attention pooling:
        scores_i = q^T x_i
        weights = softmax(scores)
        h = sum_i weights_i * x_i
    """

    def __init__(self, embed_dim):
        super().__init__()
        # Learned query vector q ∈ R^E
        self.q = nn.Parameter(torch.empty(embed_dim))
        self.reset_parameters()

    def reset_parameters(self):
        # Kaiming uniform initialization (like nn.Linear)
        nn.init.kaiming_uniform_(self.q.unsqueeze(0), a=5**0.5)
    ...
    def forward(self, x):
        ...
        # Move sequence dimension forward for dot-product
        x_seq = x.permute(0, 2, 1)  # (N, L, E)

        # Compute attention logits: (N, L)
        scores = torch.matmul(x_seq, self.q)  # (N, L)

        # Softmax over sequence length
        weights = F.softmax(scores, dim=-1)   # (N, L)

        # Weighted sum: (N, E)
        pooled = torch.sum(x_seq * weights.unsqueeze(-1), dim=1)
...
class CrossAttentionPooling(nn.Module):
    ...
    Cross-attention pooling:
        K = x_seq W_K                     (N, L, d_k)
        V = x_seq W_V                     (N, L, E)
        scores = q W_Q K^T / sqrt(d_k)   (N, 1, L)
        weights = softmax(scores)
        h = weights @ V                   (N, E)
    """

    def __init__(self, embed_dim, dim_key=None):
        super().__init__()
        if dim_key is None:
            dim_key = embed_dim
        self.scale = dim_key ** -0.5
        self.q = nn.Parameter(torch.empty(1, dim_key))
        self.W_K = nn.Linear(embed_dim, dim_key, bias=False)
        self.W_V = nn.Linear(embed_dim, embed_dim, bias=False)
        self.reset_parameters()

    def reset_parameters(self):
        nn.init.kaiming_uniform_(self.q, a=5**0.5)
        nn.init.kaiming_uniform_(self.W_K.weight, a=5**0.5)
        nn.init.kaiming_uniform_(self.W_V.weight, a=5**0.5)
    ...
    def forward(self, x):
        ...
        K = self.W_K(x_seq)             # (N, L, d_k)
        V = self.W_V(x_seq)             # (N, L, E)

        # scores: (N, 1, L)
        scores = torch.matmul(self.q, K.transpose(-2, -1)) * self.scale

        weights = F.softmax(scores, dim=-1)   # (N, 1, L)

        # (N, 1, E) -> (N, E)
        pooled = torch.matmul(weights, V).squeeze(1)
